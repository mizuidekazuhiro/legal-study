...
_PROTECTED_RE = re.compile(
    ...
    r"条(?!件)|項|号|年|月|日|円|懲役|禁錮|拘禁刑|最判|大判|判例|事件|昭和|平成|令和"
)
...
def _visible_prefix_length(text: str, compact_length: int) -> int:
    consumed = 0
    for index, char in enumerate(text):
        consumed += len(_norm(char))
        if consumed >= compact_length:
            return index + 1
    return len(text)


def _visible_suffix_start(text: str, compact_length: int) -> int:
    consumed = 0
    for index in range(len(text) - 1, -1, -1):
        consumed += len(_norm(text[index]))
        if consumed >= compact_length:
            return index
    return 0


def _trim_ocr_candidate(native: str, ocr: str, page_text: str) -> str:
    """Project the native span boundaries onto a slightly wider OCR line.

    Surgical crops intentionally include context. Remove only a short OCR
    prefix/suffix that is independently present immediately outside the native
    span in embedded page text. This preserves replacements that expand one
    corrupt glyph into multiple characters.
    """
    native_n = unicodedata.normalize("NFKC", native).strip()
    ocr_n = unicodedata.normalize("NFKC", ocr).strip()
    if not native_n or not ocr_n:
        return ocr_n
    target = native if page_text.count(native) == 1 else native.strip()
    if not target or page_text.count(target) != 1:
        return ocr_n
    start = page_text.index(target)
    before = _norm(page_text[:start])
    after = _norm(page_text[start + len(target) :])
    compact_ocr = _norm(ocr_n)

    prefix_trim = 0
    suffix_trim = 0
    max_context = min(3, len(compact_ocr) - 1)
    for size in range(1, max_context + 1):
        if before.endswith(compact_ocr[:size]):
            prefix_trim = size
        if after.startswith(compact_ocr[-size:]):
            suffix_trim = size

    left = _visible_prefix_length(ocr_n, prefix_trim) if prefix_trim else 0
    right = _visible_suffix_start(ocr_n, suffix_trim) if suffix_trim else len(ocr_n)
    if right <= left:
        return ocr_n
    candidate = ocr_n[left:right].strip()
...
def _decision_for_record(
    ...
    candidate = _trim_ocr_candidate(native, ocr, page.native_text) if native and ocr else None
